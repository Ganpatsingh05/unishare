...
import { ArrowLeft, Plus, Search, Camera,
...
const MyLostItemsPage = () => {
  ...
  const theme = {
    ...
    accent: 'text-red-600',
    accentBg: 'bg-red-50',
    accentDark: darkMode ? 'bg-red-900/20' : 'bg-red-50'
  };
  ...
  const loadLostItems = async () => {
    try {
      ...
      if (result.success) {
        ...
        const lost = (result.data || []).filter(item => item.mode === 'lost');
        setLostItems(lost);
      } else {
        setError(result.message || 'Failed to load lost items');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare lost item data
      const itemData = {
        item_name: formData.item_name,
        description: formData.description,
        mode: 'lost',
        where_last_seen: formData.where_last_seen,
        date_lost: formData.date_lost,
        time_lost: formData.time_lost || null,
      ...
      if (selectedItem) {
        ...
        if (result.success) {
          ...
          showMessage('Lost item updated successfully!', 'success');
        }
      } else {
        // Create new item
        result = await createLostFoundItem(itemData, selectedImages);
        if (result.success && result.data) {
          setLostItems(prev => [result.data, ...prev]);
          showMessage('Lost item reported successfully!', 'success');
        }
      }

      if (!result.success) {
        throw new Error(result.message || 'Failed to save lost item');
      }
      ...
      showMessage(error.message || 'Failed to save lost item', 'error');
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showMessage('Lost item deleted successfully!', 'success');
        setIsDeleteModalOpen(false);
        setItemToDelete(null);
      } else {
        throw new Error(result.message || 'Failed to delete lost item');
      }
    } catch (error) {
      console.error('Error deleting lost item:', error);
      showMessage(error.message || 'Failed to delete lost item', 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className={`min-h-screen ${theme.bg} flex items-center justify-center`}>
        <div className="text-center">
          <h1 className={`text-2xl font-bold ${theme.text} mb-4`}>Please log in to view your lost items</h1>
        ...
        <div className="max-w-5xl mx-auto px-4 py-6">
          ...
          <div className="mt-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className={`text-3xl font-bold ${theme.text}`}>My Lost Items</h1>
            </div>
            <p className={`${theme.textMuted}`}>Manage your lost item reports</p>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className={theme.textMuted}>Loading lost items...</p>
          </div>
        ) : lostItems.length === 0 ? (
          <div className="col-span-full flex flex-col items-center justify-center py-16 text-center">
            <Search className="w-16 h-16 text-gray-400 mb-4" />
            <h3 className={`text-lg font-semibold ${theme.text} mb-2`}>No Lost Items Reported</h3>
            <p className={`${theme.textMuted} mb-6`}>You haven't reported any lost items yet. Start by reporting your first lost item.</p>
            <button
              onClick={handleCreate}
          ...
          <div className={`${theme.card} rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto`}>
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl font-semibold ${theme.text}`}>
                {selectedItem ? 'Edit Lost Item' : 'Report Lost Item'}
              </h2>
              <button
                onClick={() => {
                  setIsCreateModalOpen(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`block text-sm font-medium ${theme.text} mb-1`}>Last Seen Location *</label>
                  <input
                    type="text"
                    value={formData.where_last_seen}
                    onChange={(e) => setFormData(prev => ({...prev, where_last_seen: e.target.value}))}
                    className={`w-full p-2 border rounded-lg ${theme.border} ${theme.card}`}
                ...
                <div>
                  <label className={`block text-sm font-medium ${theme.text} mb-1`}>Date Lost *</label>
                  <input
                    type="date"
                    value={formData.date_lost}
                    onChange={(e) => setFormData(prev => ({...prev, date_lost: e.target.value}))}
                    className={`w-full p-2 border rounded-lg ${theme.border} ${theme.card}`}
                    required
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`block text-sm font-medium ${theme.text} mb-1`}>Time Lost (Optional)</label>
                  <input
                    type="time"
                    value={formData.time_lost}
                    onChange={(e) => setFormData(prev => ({...prev, time_lost: e.target.value}))}
                    className={`w-full p-2 border rounded-lg ${theme.border} ${theme.card}`}
                  />
                </div>
                <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`${theme.card} rounded-lg p-6 m-4 max-w-md w-full`}>
            <div className="flex justify-between items-center mb-4">
              <h2 className={`text-lg font-semibold ${theme.text}`}>Delete Lost Item</h2>
              <button
                onClick={() => {
                  setIsDeleteModalOpen(false);
                  setItemToDelete(null);
